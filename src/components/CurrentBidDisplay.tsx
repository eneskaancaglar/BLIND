...
export function CurrentBidDisplay({
  ...
  return (
    <div className={`bid-display flex items-center ${strip ? "flex-row gap-2" : "flex-col gap-1"}`}>
      <div className={`bid-display-row flex items-center justify-center ${strip ? "gap-1.5" : "gap-2 sm:gap-3"}`}>
        <PlayingCard
          card={{ rank: bid.rank, suit: demoSuit() }}
          size={cardSize}
          tilt="flat"
        />
        <span className={`bid-display-count ${strip ? "bid-display-count-strip" : ""}`}>×{bid.count}</span>
      </div>
      {playerName ? (
        <p
